...
class _PdfViewerScreenState extends State<PdfViewerScreen> {
  ...
  Future<void> _saveAnnotationsToHive() async {
    try {
      ...
      for (var entry in _pageComments.entries) {
        if (entry.value.isNotEmpty) {
          final serializedComments = entry.value.map((c) => c.toJson()).toList();
          await box.put('${widget.pdfId}_${entry.key}_comments', serializedComments);
        } else {
  ...
  Future<void> _loadAnnotationsForPage(int pageNumber) async {
    ...
    if (!_pageDrawings.containsKey(pageNumber)) {
      ...
      if (data != null) {
        lines = (data as List<dynamic>).map((e) => DrawingLine.fromJson(Map<String, dynamic>.from(e))).toList();
      }
    ...
    if (!_pageComments.containsKey(pageNumber)) {
      final dynamic commentData = box.get('${widget.pdfId}_${pageNumber}_comments');
      List<CommentModel> comments = [];
      if (commentData != null) {
        comments = (commentData as List<dynamic>).map((e) => CommentModel.fromJson(Map<String, dynamic>.from(e))).toList();
      }
                  ...
                  children: [
                    ...
                    IgnorePointer(
                      ...
                      child: GestureDetector(
                        ...
                        onPanStart: (details) {
                          if (_selectedTool != 3) _startStroke(details, context, pageRect, page);
                        },
                        onPanUpdate: (details) {
  ...
  Widget _toolIcon(IconData icon, int index) {
    return Container(
      decoration: BoxDecoration(
        color: _selectedTool == index ? AppColors.accentYellow.withOpacity(0.2) : Colors.transparent,
        borderRadius: BorderRadius.circular(8)
      ),
      child: IconButton(
